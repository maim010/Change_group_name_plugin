...
"""
    ...
    action_parameters = {
        "target": "禁言或解除禁言的对象，必填，输入你要禁言或解除禁言的对象的名字，请仔细思考不要弄错对象",
        "duration": "禁言时长（秒），默认3600秒（1小时），范围0-2592000秒（30天），为0则表示解除禁言",
        "reason": "禁言理由，可选",
    }
    ...
    def _check_group_permission(self) -> Tuple[bool, Optional[str]]:
        if not self.is_group:
            return False, "禁言或解除禁言动作只能在群聊中使用"
        allowed_groups = self.get_config("permissions.allowed_groups", [])
        if not allowed_groups:
            logger.info(f"{self.log_prefix} 群组权限未配置，允许所有群使用禁言或解除禁言动作")
            return True, None
        current_group_key = f"{self.platform}:{self.group_id}"
        for allowed_group in allowed_groups:
            if allowed_group == current_group_key:
                logger.info(f"{self.log_prefix} 群组 {current_group_key} 有禁言或解除禁言动作权限")
                return True, None
        logger.warning(f"{self.log_prefix} 群组 {current_group_key} 没有禁言或解除禁言动作权限")
        return False, "当前群组没有使用禁言或解除禁言动作的权限"

    async def execute(self) -> Tuple[bool, Optional[str]]:
        logger.info(f"{self.log_prefix} 执行智能禁言或解除禁言动作")
        ...
        if not target:
            error_msg = "禁言或解除禁言目标不能为空"
            logger.error(f"{self.log_prefix} {error_msg}")
            await self.send_text("没有指定禁言或解除禁言对象呢~")
        ...
        if not has_permission:
            ...
            result_status, result_message = await generator_api.rewrite_reply(
                ...
                reply_data={
                    "raw_reply": "我想禁言或解除禁言{target}，但是我没有权限",
                    "reason": "表达自己没有在这个群禁言或解除禁言的能力",
                },
            )
            if result_status:
            ...
            await self.store_action_info(
                ...
                action_prompt_display=f"尝试禁言或解除禁言用户 {target}，但是没有权限，无法操作",
                action_done=True,
            )
        ...
        if not group_id:
            error_msg = "无法获取群聊ID"
            logger.error(f"{self.log_prefix} {error_msg}")
            await self.send_text("执行禁言或解除禁言动作失败（群ID缺失）")
            return False, error_msg
        # Napcat API 禁言实现
        import httpx
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if resp_json.get("status") == "ok" and resp_json.get("retcode") == 0:
                    logger.info(f"{self.log_prefix} 成功禁言或解除禁言 {target}({user_id})，群: {group_id}，时长: {duration}秒")
                    await self.store_action_info(
                        action_build_into_prompt=True,
                        action_prompt_display=f"尝试禁言或解除禁言用户 {target}，时长：{duration}秒，原因：{reason}",
                        action_done=True,
                    )
                    return True, f"成功禁言或解除禁言 {target}"
                else:
                    error_msg = f"Napcat API返回失败: {resp_json}"
                    logger.error(f"{self.log_prefix} {error_msg}")
                    await self.send_text("执行禁言或解除禁言动作失败（API返回失败）")
                    return False, error_msg
            else:
                error_msg = f"Napcat API请求失败: HTTP {response.status_code}"
                logger.error(f"{self.log_prefix} {error_msg}")
                await self.send_text("执行禁言或解除禁言动作失败（API请求失败）")
                return False, error_msg
        except Exception as e:
            error_msg = f"Napcat API请求异常: {e}"
            logger.error(f"{self.log_prefix} {error_msg}")
            await self.send_text("执行禁言或解除禁言动作失败（API异常）")
            return False, error_msg
...
class MuteUserCommand(BaseCommand):
    ...
    command_description = "禁言或解除禁言命令，手动执行禁言或解除禁言操作"
    command_pattern = r"^/mute\s+(?P<target>\S+)(?:\s+(?P<duration>\d+))?(?:\s+(?P<reason>.+))?$"
    command_help = "禁言或解除禁言指定用户，用法：/mute <用户名> [时长(秒)] [理由]"
    ...
    def _check_user_permission(self) -> Tuple[bool, Optional[str]]:
        ...
        if not allowed_users:
            logger.info(f"{self.log_prefix} 用户权限未配置，允许所有用户使用禁言或解除禁言命令")
            return True, None
        current_user_key = f"{current_platform}:{current_user_id}"
        for allowed_user in allowed_users:
            if allowed_user == current_user_key:
                logger.info(f"{self.log_prefix} 用户 {current_user_key} 有禁言或解除禁言命令权限")
                return True, None
        logger.warning(f"{self.log_prefix} 用户 {current_user_key} 没有禁言或解除禁言命令权限")
        return False, "你没有使用禁言或解除禁言命令的权限"
    ...
    async def execute(self) -> Tuple[bool, Optional[str]]:
        try:
            ...
            if not user_id or user_id == "unknown":
                error_msg = f"未找到用户 {target} 的ID，请输入person_name进行禁言或解除禁言"
                await self.send_text(f"❌ 找不到用户 {target} 的ID，请输入person_name进行禁言或解除禁言，而不是qq号或者昵称")
                logger.error(f"{self.log_prefix} {error_msg}")
                return False, error_msg
            logger.info(f"{self.log_prefix} 执行禁言或解除禁言命令: {target}({user_id})，时长: {duration}秒")
            # 发送群聊禁言命令（使用 NapCat API）
            from src.plugin_system.apis import send_api
            ...
            logger.info(f"{self.log_prefix} Napcat禁言或解除禁言API请求: {napcat_api}, payload={payload}")
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.post(napcat_api, json=payload, timeout=5)
                logger.info(f"{self.log_prefix} Napcat禁言或解除禁言API响应: status={response.status_code}, body={response.text}")
                if response.status_code == 200:
                    resp_json = response.json()
                    if resp_json.get("status") == "ok" and resp_json.get("retcode") == 0:
                        # 将秒转换为更易读的格式
                        ...
                        error_msg = f"Napcat API返回失败: {resp_json}"
                        logger.error(f"{self.log_prefix} {error_msg}")
                        await self.send_text("❌ 发送禁言或解除禁言命令失败（API返回失败）")
                        return False, error_msg
                else:
                    error_msg = f"Napcat API请求失败: HTTP {response.status_code}"
                    logger.error(f"{self.log_prefix} {error_msg}")
                    await self.send_text("❌ 发送禁言或解除禁言命令失败（API请求失败）")
                    return False, error_msg
            except Exception as e:
                error_msg = f"Napcat API请求异常: {e}"
                logger.error(f"{self.log_prefix} {error_msg}")
                await self.send_text("❌ 发送禁言或解除禁言命令失败（API异常）")
                return False, error_msg
        except Exception as e:
            logger.error(f"{self.log_prefix} 禁言或解除禁言命令执行失败: {e}")
            await self.send_text(f"❌ 禁言或解除禁言命令错误: {str(e)}")
            return False, str(e)
...
class MuteUserPlugin(BasePlugin):
    ...
    config_schema = {
        ...
        "components": {
            "enable_smart_mute": ConfigField(type=bool, default=True, description="是否启用智能禁言或解除禁言Action"),
            "enable_mute_command": ConfigField(
                type=bool, default=False, description="是否启用禁言或解除禁言命令Command（调试用）"
            ),
        },
        "permissions": {
            "allowed_users": ConfigField(
                type=list,
                default=[],
                description="允许使用禁言或解除禁言命令的用户列表，格式：['platform:user_id']，如['qq:123456789']。空列表表示不启用权限控制",
            ),
            "allowed_groups": ConfigField(
                type=list,
                default=[],
                description="允许使用禁言或解除禁言动作的群组列表，格式：['platform:group_id']，如['qq:987654321']。空列表表示不启用权限控制",
            ),
        },
        "mute": {
            ...
            "templates": ConfigField(
                type=list,
                default=[
                    "好的，已将 {target} 禁言或解除禁言 {duration}，理由：{reason}",
                    "收到，对 {target} 执行禁言或解除禁言 {duration}，因为{reason}",
                    "明白了，禁言 {target} {duration}，原因是{reason}",
                    "已将 {target} 禁言或解除禁言 {duration}，理由：{reason}",
                    "对 {target} 执行禁言或解除禁言 {duration}，因为{reason}",
                ],
            ...
            "error_messages": ConfigField(
                ...
                description="执行禁言或解除禁言过程中发生错误时发送的随机消息模板",
            ),
        },
        "smart_mute": {
